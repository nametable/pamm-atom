...
function jsSortOnlineMods() {
	switch (settings.sort()) {
		case "LAST_UPDATED":
			objOnlineMods.sort(sortModBy('date', false, function(x) {
				return new Date(x ? x : 0);
			}));
...
function jsGenerateModEntryHTML(objMod, boolIsInstalled) {
	...
	var modOnline = _.find(objOnlineMods, function(mod) {
		return mod.identifier === id
	});
	var modInstalled = _.find(objInstalledMods.union, function(mod) {
		return mod.identifier === id
	});
...
function jsGenerateInstalledModsListHTML(context) {
	...
	$("#filter_area_" + installedcontext + "_text_filter").on("keyup", function() {
...
function jsSetLogLevel(intLevel) {
	...
	if (intLevel != intLogLevel) {
		...
		$(".log_level3").css({
			display: intLogLevel >= 3 ? "block" : "none"
		});
		$(".log_level4").css({
			display: intLogLevel >= 4 ? "block" : "none"
		});
	}
}
	...
	jsDownload(PA_VERSION_URL, {
		success: function(build) {
			if (build !== pa.streams.stable.build) {
				...
				setTimeout(function() {
					alert("PA update available: " + build + "\nUse the UberLauncher to update your installation.");
				}, 1);
			} else {
...
function RestartPAMM() {
	...
	var child = child_process.spawn(argv[0], argv.splice(1), {
		detached: true,
		stdio: 'inherit'
	});
	child.unref();

	...
	localDeferInstalledMods.done(function() {
		deferred.resolve(
			_.filter(
				_.toArray(installed),
				function(mod) {
					return !isBuiltinMod(mod.identifier) && mod.context === context
				}
			)
		);
	}).fail(function(err) {
	...
	var _install = function(ids, callback) {
		...
		if (fs.existsSync(installpath)) {
			...
			if (fs.existsSync(modinfopath)) {
				var modinfo = fs.readFileSync(modinfopath, {
					encoding: 'utf8'
				});
...
var findInstalledMods = function() {
	...
	var _loadMountedMods = function(context) {
		...
		if (fs.existsSync(modsjsonpath)) {
			var modsjson = fs.readFileSync(modsjsonpath, {
				encoding: 'utf8'
			});
		...
		for (var i = 0; i < moddirs.length; ++i) {
			...
			if (fs.statSync(moddir).isDirectory()) {
				try {
					...
					var strmodinfo = fs.readFileSync(modinfopath, {
						encoding: 'utf8'
					});
					...
					if (mod.enabled === false) {
						...
						mod.enabled = true;
						fs.writeFileSync(modinfopath, JSON.stringify(mod, null, 4), {
							encoding: 'utf8'
						});
					}
...
var _updateFiles = function(context) {
	...
	var enabledmods = _.sortBy(
		_.filter(
			_.toArray(installed),
			function(mod) {
				return mod.enabled !== false && mod.context === context;
			}
		),
		function(mod) {
			return mod.priority
	...
	_.each(scenes, function(scene) {
		scenemodlist[scene] = [];
	}); // temp fix for PA Stats => all scenes must be initialized by empty an array
	...
	var mount_order = _.map(enabledmods, 'identifier'); // may be modified for server mods
	...
	switch (context) {
		...
		case 'client':
			...
			fs.writeFileSync(
				path.join(pamm_path, 'ui/mods/ui_mod_list_for_server.js'), uimodlist, {
					encoding: 'utf8'
				}
			);
			...
			fs.writeFileSync(
				path.join(pamm_path, 'ui/mods/mods_list.json'), JSON.stringify(installed, null, 4), {
					encoding: 'utf8'
				}
			);
	...
	if (uimodlist) {
		...
		fs.writeFileSync(
			path.join(pamm_path, 'ui/mods/ui_mod_list.js'), uimodlist, {
				encoding: 'utf8'
			}
		);
	}
	...
	fs.writeFileSync(
		path.join(pa.modspath[context], 'mods.json'), JSON.stringify(mods, null, 4), {
			encoding: 'utf8'
		}
	);
};
